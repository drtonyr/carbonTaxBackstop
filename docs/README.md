...
The ONS data is segmented into income deciles, so elasicity can be estimated using all three functions. The appendix contanis plots for all expenditure items and one is given below.   As can be seen there is no clear best funciton, so for simpliity a linear relationship is used in the remainder of this work.
